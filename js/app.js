...
function parseConfigInput(text) {
  try {
    // Strip JS variable assignment if present
    const cleaned = text
      .replace(/^[\s\S]*?const\s+\w+\s*=\s*/, '')
      .replace(/^[\s\S]*?var\s+\w+\s*=\s*/, '')
      .replace(/;[\s\S]*$/, '')
      .trim();
...
function bindSetupEvents() {
  document.getElementById('saveConfigBtn').addEventListener('click', async () => {
    ...
    if (!cfg || !cfg.apiKey || !cfg.databaseURL) {
      errEl.textContent = '❌ Config ঠিকমতো পেস্ট করুন। databaseURL অবশ্যই থাকতে হবে।';
